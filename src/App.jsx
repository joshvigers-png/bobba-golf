          ...
          <div className="sheet" onClick={e => e.stopPropagation()}>
            ...
            {(user.friendRequests || []).length > 0 ? (
              <>
                ...
                <button className="btn btn-primary" onClick={() => { setNotificationsOpen(false); onOpenModule("community"); }}>View in The Lounge</button>
              </>
            ) : (
              <>
...
// ─── Scan Scorecard Step ─────────────────────────────────────────────────────
// Extracted as a standalone component so React hooks (useState, useRef) are
...
function ScanScorecardStep({ onBack, onConfirm, proxyUrl, existingCourse }) {
  ...
  const handleImage = (e) => {
    ...
    reader.onload = () => {
      setScanImage(reader.result.split(",")[1]);
  ...
  const doScan = async () => {
    if (!scanImage) { setScanError("Please select a photo of the scorecard first."); return; }
    if (!scanName.trim()) { setScanError("Please enter the course name."); return; }
    setScanError("");
    setScanning(true);
    try {
      const res = await fetch(`${proxyUrl}/ai-vision`, {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              ...
              { type: "text", text: `This is a golf scorecard. Extract the hole data and return ONLY a valid JSON object with no markdown, no explanation, just raw JSON in this exact format:
{
  "tees": [
...
- Return ONLY the JSON, nothing else` }
  ...
  const confirmScan = () => {
    ...
    const tees = {};
    scanned.tees.forEach((t, i) => { tees[teeKeys[i]] = { rating: t.rating, slope: t.slope }; });
    const holes = scanned.holes.map(h => ({ n: h.n, par: h.par ?? 4, si: h.si ?? h.n, yds: h.yds || {} }));
    onConfirm({ id: `scan_${Date.now()}`, name: scanName.trim(), location: "", tees, holes, fromScan: true });
  };
      ...
      <div style={{ padding: "0 18px" }}>
        ...
        {scanPreview ? (
          <div style={{ marginBottom: 16 }}>
            ...
            <label style={{ display: "block", marginTop: 10, textAlign: "center", padding: "12px 0", border: `1.5px solid ${C.line}`, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
              Retake Photo
              <input type="file" accept="image/*" style={{ display: "none" }} onChange={handleImage} />
            </label>
          </div>
        ) : (
          <label style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, width: "100%", marginBottom: 16, padding: "24px 0", fontSize: 14, fontWeight: 700, border: `1.5px solid ${C.line}`, cursor: "pointer", background: C.white }}>
        ...
        {scanned && (
          <div style={{ marginTop: 8 }}>
            ...
            <div style={{ background: C.white, border: `1px solid ${C.line}`, marginBottom: 16, overflow: "hidden" }}>
              ...
              {scanned.holes.map(h => (
                <div key={h.n} style={{ display: "grid", gridTemplateColumns: "32px 36px 36px auto", padding: "7px 12px", borderBottom: `1px solid ${C.line}`, fontSize: 12 }}>
                  <div style={{ fontWeight: 800 }}>{h.n}</div>
                  <div>{h.par ?? "—"}</div>
                  <div style={{ color: h.si ? C.black : C.ash }}>{h.si ?? "—"}</div>
                  <div style={{ color: C.steel, fontSize: 11 }}>{h.yds ? Object.entries(h.yds).filter(([,v]) => v).map(([k,v]) => `${k}: ${v}y`).join(" · ") : "—"}</div>
                </div>
              ))}
            </div>
  ...
  return (
    <div style={{ background: C.paper, minHeight: "100vh", paddingBottom: 100 }}>
      ...
      {comp.players.map(player => {
        ...
        // Match play hole winner indicator — works for both 2-ball singles
        // and 4-ball pairs (best net score within each pair represents the
        // team for that hole).
        const matchHoleResult = () => {
          ...
          if (comp.ballCount === 4) {
            const team1 = [comp.players[0], comp.players[1]];
            const team2 = [comp.players[2], comp.players[3]];
            const nets1 = team1.map(p => netScore(p, hole));
            const nets2 = team2.map(p => netScore(p, hole));
            // Wait for all 4 scores — a partial "best so far" could flip once
            // a teammate who hasn't played the hole yet enters their score.
            ...
            if (onTeam1) return n1 < n2 ? "won" : "lost";
            return n2 < n1 ? "won" : "lost";
          }
          return null;
        };
